...
int main(int argc, char *argv[])
{
    ...
    TH1D* h_HL_dt = new TH1D("h_HL_dt", "Time Difference Between Subsequent High Light Events, Shifted by 16.67ms", 180, -1440, 1440);
    ...
    int any_event_counter = 0;

    int PMT_event_counter = 0;

    int SiPM_event_counter = 0;
    ...
    ofstream BBVFile(Form("Background_Bin_Vals_Run%i.txt", run));
    ...
    for (size_t iRun = 0; iRun < runlist.size(); iRun++) {
        ...
        if (strcmp(outputFilePath, "") == 0) {
            outputFile.Form("%s.root", Form("event_analysis_week%i_run%i", run, run_iterable));
        } else {
            outputFile.Form("%s/%s.root", outputFilePath, Form("event_analysis_week%i_run%i", run, run_iterable));
        }
        ...
        Bool_t *br_re;      /* Tacks whether most recent Event 61 event had a large waveform peak or not */
        ...
        eventTree->Branch("re", &br_re, "re/O");
        ...
        std::vector<struct pulse> LL_vec;
        ...
        for (int iEnt = 0; iEnt < t->GetEntries(); iEnt++) {
            ...
            // if (avg_pulse.beam) {h_HL_dt->Fill(((nsTime + chan_15_start) - HL_time) - 16666667); HL_time = nsTime + chan_15_start;}

            // After every low light LED event, enable recording of vector pulses

            if (avg_pulse.trigger == 16) {record_pulses = true; LL_time = avg_pulse.start; LL_vec.push_back(avg_pulse);}
            
            // After every high light LED event, disable recording of vector pulses
            
            else if (avg_pulse.trigger == 8) {record_pulses = false; LL_vec.clear(); h_HL_dt->Fill((avg_pulse.start - HL_time) - 16666667); HL_time = avg_pulse.start;}
            ...
            if (record_pulses && avg_pulse.trigger != 0 && avg_pulse.trigger != 4 && avg_pulse.trigger != 8 && avg_pulse.trigger != 16) {         // * (currentHLmean / defaultHLmean)
                ...
                if (avg_pulse.number >= 10 && avg_pulse.energy >= 50 && avg_pulse.start - last_event_time > echo_cut) {
                    ...
                    // if (p_int_16 <= 150 && p_int_17 <= 150 && p_int_18 <= vp18intcut && p_int_19 <= 150 && p_int_20 <= 150 && p_int_21 <= 150 && p_int_22 <= 150 && p_int_23 <= 150 && avg_pulse.vp_int_top <= 250) {
            ...
            if (record_pulses && avg_pulse.selection_cut >= 5) {

                // Record most recent low light LED event

                if (!record_ext_trig) {

                    Int_t fnum = LL_vec.back().file_num;
                    Int_t entry = LL_vec.back().entry;
                    Long64_t utime = LL_vec.back().unix_time;
                    Long64_t st = LL_vec.back().start;
                    Long64_t ed = LL_vec.back().end;
                    Double_t pk = LL_vec.back().peak;
                    Double_t ey = LL_vec.back().energy;
                    Int_t nr = LL_vec.back().number;
                    Bool_t se = LL_vec.back().single;
                    Bool_t bm = LL_vec.back().beam;
                    Bool_t re = LL_vec.back().recent_ev61;
                    Int_t tr = LL_vec.back().trigger;
                    Int_t lh = LL_vec.back().length;
                    Double_t svpe = LL_vec.back().side_vp_energy;
                    Double_t tvpe = LL_vec.back().top_vp_energy;
                    Double_t avpe = LL_vec.back().all_vp_energy;
                    Long64_t lct = LL_vec.back().last_cosmics_time;
                    Long64_t ldt = LL_vec.back().last_det_time;
                    Int_t selcut = LL_vec.back().selection_cut;
                    // Potential issues, any e < 0 or pk > maxValue or waveforms abnormally sized or peaks too spread out
                    Bool_t issue = false;
                    if (ey < 0 || pk >= maxPeak || lh != 45 || !se || tr != 16) {issue = true;}
                    Long64_t wftime = LL_vec.back().wf_time;
                    Long64_t vptime16 = LL_vec.back().vp_start_16;
                    Long64_t vptime17 = LL_vec.back().vp_start_17;
                    Long64_t vptime18 = LL_vec.back().vp_start_18;
                    Long64_t vptime19 = LL_vec.back().vp_start_19;
                    Long64_t vptime20 = LL_vec.back().vp_start_20;
                    Long64_t vptime21 = LL_vec.back().vp_start_21;
                    Long64_t vptime22 = LL_vec.back().vp_start_22;
                    Long64_t vptime23 = LL_vec.back().vp_start_23;
                    Long64_t vptimetop = LL_vec.back().vp_start_top;
                    Long64_t vpint16 = LL_vec.back().vp_int_16;
                    Long64_t vpint17 = LL_vec.back().vp_int_17;
                    Long64_t vpint18 = LL_vec.back().vp_int_18;
                    Long64_t vpint19 = LL_vec.back().vp_int_19;
                    Long64_t vpint20 = LL_vec.back().vp_int_20;
                    Long64_t vpint21 = LL_vec.back().vp_int_21;
                    Long64_t vpint22 = LL_vec.back().vp_int_22;
                    Long64_t vpint23 = LL_vec.back().vp_int_23;
                    Long64_t vpinttop = LL_vec.back().vp_int_top;
                    ...
                    eventTree->SetBranchAddress("re", &re);
                ...
                record_ext_trig = true;

                // Record current internally triggered event

                Int_t fnum = avg_pulse.file_num;
                Int_t entry = avg_pulse.entry;
                Long64_t utime = avg_pulse.unix_time;
                Long64_t st = avg_pulse.start;
                Long64_t ed = avg_pulse.end;
                Double_t pk = avg_pulse.peak;
                Double_t ey = avg_pulse.energy;
                Int_t nr = avg_pulse.number;
                Bool_t se = avg_pulse.single;
                Bool_t bm = avg_pulse.beam;
                Bool_t re = avg_pulse.recent_ev61;
                Int_t tr = avg_pulse.trigger;
                Int_t lh = avg_pulse.length;
                Double_t svpe = avg_pulse.side_vp_energy;
                Double_t tvpe = avg_pulse.top_vp_energy;
                Double_t avpe = avg_pulse.all_vp_energy;
                Long64_t lct = avg_pulse.last_cosmics_time;
                Long64_t ldt = avg_pulse.last_det_time;
                Int_t selcut = avg_pulse.selection_cut;
                // Potential issues, any e < 0 or pk > maxValue or waveforms abnormally sized or peaks too spread out
                Bool_t issue = false;
                if (ey < 0 || pk >= maxPeak || lh != 45 || !se || (tr != 2 && tr != 34)) {issue = true;}
                Long64_t wftime = avg_pulse.wf_time;
                Long64_t vptime16 = avg_pulse.vp_start_16;
                Long64_t vptime17 = avg_pulse.vp_start_17;
                Long64_t vptime18 = avg_pulse.vp_start_18;
                Long64_t vptime19 = avg_pulse.vp_start_19;
                Long64_t vptime20 = avg_pulse.vp_start_20;
                Long64_t vptime21 = avg_pulse.vp_start_21;
                Long64_t vptime22 = avg_pulse.vp_start_22;
                Long64_t vptime23 = avg_pulse.vp_start_23;
                Long64_t vptimetop = avg_pulse.vp_start_top;
                Long64_t vpint16 = avg_pulse.vp_int_16;
                Long64_t vpint17 = avg_pulse.vp_int_17;
                Long64_t vpint18 = avg_pulse.vp_int_18;
                Long64_t vpint19 = avg_pulse.vp_int_19;
                Long64_t vpint20 = avg_pulse.vp_int_20;
                Long64_t vpint21 = avg_pulse.vp_int_21;
                Long64_t vpint22 = avg_pulse.vp_int_22;
                Long64_t vpint23 = avg_pulse.vp_int_23;
                Long64_t vpinttop = avg_pulse.vp_int_top;
                ...
                for (int iVec = 0; iVec < event_bin_val.size(); iVec++) {BBVFile << event_bin_val[iVec] << "\t";} BBVFile << avg_pulse.start - last_int_trig_event_time << "\t" << "\n";

            }

            if (record_ext_trig && avg_pulse.trigger == 8) {

                // Record current high light LED event
                ...
                // Potential issues, any e < 0 or pk > maxValue or waveforms abnormally sized or peaks too spread out
                Bool_t issue = false;
                if (ey < 0 || pk >= maxPeak || lh != 45 || !se || tr != 8) {issue = true;}
                Long64_t wftime = avg_pulse.wf_time;
                Long64_t vptime16 = avg_pulse.vp_start_16;
                Long64_t vptime17 = avg_pulse.vp_start_17;
                Long64_t vptime18 = avg_pulse.vp_start_18;
            ...
            if (avg_pulse.beam && (avg_pulse.trigger == 1 || avg_pulse.trigger == 3 || avg_pulse.trigger == 33 || avg_pulse.trigger == 35)) {avg_pulse.recent_ev61 = true;}

            else if (!avg_pulse.beam && (avg_pulse.trigger == 1 || avg_pulse.trigger == 3 || avg_pulse.trigger == 33 || avg_pulse.trigger == 35)) {avg_pulse.recent_ev61 = false;}
            ...
            // Calculate deadtime from all events

            if (!avg_pulse.beam && avg_pulse.trigger != 4 && avg_pulse.trigger != 8 && avg_pulse.trigger != 16) {

                any_event_counter += 1;

                deadtime_count += echo_cut;
                
                if (avg_pulse.number >= 10 && avg_pulse.energy >= 50) {

                    PMT_event_counter += 1;

                    deadtime_count += (muon_dt_max - echo_cut);

                }

                else if (pulse_at_end && (p_int_16 > min_muon_vals[0] / 2 || p_int_17 > min_muon_vals[1] / 2 || p_int_18 > min_muon_vals[2] / 2 || p_int_19 > min_muon_vals[3] / 2 || p_int_20 > min_muon_vals[4] / 2 || p_int_21 > min_muon_vals[5] / 2 || p_int_22 > min_muon_vals[6] / 2 || p_int_23 > min_muon_vals[7] / 2 || avg_pulse.vp_int_top > min_muon_vals[8] / 2)) {
                    
                    SiPM_event_counter += 1;
    ...
    TCanvas* c_HL_dt = new TCanvas("c_HL_dt", "Subsequent HL dts", 1200, 700);
    c_HL_dt->SetLogy();
    c_HL_dt->cd();
    h_HL_dt->GetXaxis()->SetTitle("Delta-T (ns)");
    h_HL_dt->GetYaxis()->SetTitle("Counts");
    h_HL_dt->Draw();
    c_HL_dt->SaveAs(Form("/data7/coherent/data/d2o/emward/Detector_Data_Analysis/plots/h_HL_dt_%i.png", run));
    // c_HL_dt->SaveAs("/mnt/c/Users/eliwa/OneDrive/Documents/Detector_Data_Analysis/plots/h_HL_dt.png");
    c_HL_dt->Close();
    // gPad->Print(Form("/data7/coherent/data/d2o/emward/Detector_Data_Analysis/h_HL_dt.png"));
    // gPad->Print(Form("/mnt/c/Users/eliwa/Documents/Detector_Data_Analysis/plots/h_HL_dt.png"));
    h_HL_dt->Reset();
    ...
    cout << "\n" << "Total accumulated deadtime = " << deadtime_count << endl;

    cout << "\n" << "Number of internally triggered events = " << any_event_counter << endl;

    cout << "\n" << "Number of PMT peak events = " << PMT_event_counter << endl;

    cout << "\n" << "Number of SiPM peak events = " << SiPM_event_counter << endl;
